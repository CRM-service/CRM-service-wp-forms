 ...
 * @Last Modified time: 2018-04-06 10:47:35
 *
 * @package crmservice
 */
...
<div class="crmservice-metabox-settings-wrap">
	<div class="row row-setting">
		<div class="col col-form">
			...
			<?php if ( empty( $forms ) ) : ?>
				<p><?php printf( wp_kses( __( 'You do not have any forms yet. Create a <a href="%s">new form</a>.', 'crmservice' ), array(
					'a' => array(
						'href' => array(),
					),
				) ), esc_url( $new_form_url ) ) ?></p>
			<?php else : ?>
				...
				<div class="col col-module">
					<select name="crmservice_connections[<?php echo $connection_id ?>][module_field]">
						...
						<?php foreach ( $module_fields as $module_field_id => $module_field ) : ?>
							<option value="<?php echo $module_field->name ?>"<?php if ( isset( $connection['module_field'] ) && $connection['module_field'] === $module_field->name ) { echo ' selected'; } ?>><?php echo $module_field->label ?></option>
						<?php endforeach; ?>
					</select>
